<template>
    <div class="pa-10">
      ...
      <v-row>
        ...
        <v-col cols="1">
          <!-- Filter button -->
          <v-icon @click="showFilterDialog" class="mt-5 mb-12" color="#23ffe5" large>
            mdi-filter
          </v-icon>
        </v-col>
      </v-row>
  ...
  const API_KEY = 'a6a07bdb1ae12672ae2d301063d83c40';
  ...
  export default {
    ...
    methods: {
        ...
        try {
          const fetchPage = async (page) => {
            ...
            if (query) {
              ...
              url = `https://api.themoviedb.org/3/search/${searchType}?api_key=${API_KEY}&query=${encodeURIComponent(query)}&with_watch_providers=337&watch_region=US&page=${page}`;
            } else {
              const type = 'movie'; 
              ...
              const disneyAnimationFilter = `&with_watch_providers=337&watch_region=US&with_genres=16`;
              url = `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}${yearParam}${languageParam}${ratingParam}${sortParam}${disneyAnimationFilter}&page=${page}`;
            }
      ...
      async fetchDefaultMedia() {
        ...
        try {
          const fetchPage = async (page) => {
            const url = `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&with_watch_providers=337&watch_region=US&with_genres=16&page=${page}`;
            const response = await axios.get(url);
            return response.data.results;
          };
