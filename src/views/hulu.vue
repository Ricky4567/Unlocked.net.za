  ...
  export default {
    ...
    methods: {
        ...
        try {
          const fetchPage = async (page) => {
            ...
            if (query) {
              ...
              url = `https://api.themoviedb.org/3/search/${searchType}?api_key=${API_KEY}&query=${encodeURIComponent(query)}&with_watch_providers=15&watch_region=US&page=${page}`;
            } else {
              ...
              const huluFilter = `&with_watch_providers=15&watch_region=US`;
              url = `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}${yearParam}${languageParam}${ratingParam}${sortParam}${huluFilter}&page=${page}`;
            }
      ...
      async fetchDefaultMedia() {
        ...
        try {
          const fetchPage = async (page) => {
            const url = `https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&with_watch_providers=15&watch_region=US&page=${page}`;
            const response = await axios.get(url);
            return response.data.results;
          };
