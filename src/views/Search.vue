    ...
    async fetchMedia(query = '') {
      ...
      try {
        const fetchPage = async (page) => {
          let url;
          if (query) {
            url = `https://api.themoviedb.org/3/search/${this.selectedType || 'multi'}?api_key=${apiKey}&query=${encodeURIComponent(query)}&page=${page}`;
          } else {
            ...
            url = `https://api.themoviedb.org/3/discover/${type}?api_key=${apiKey}&with_genres=${this.selectedGenres.join(',')}${yearParam}${languageParam}${ratingParam}${sortParam}&page=${page}`;
          }
