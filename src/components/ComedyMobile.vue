<template>
    <div class="scroll-container">
        <v-row class="mt-3">
        <div class="white--text my-3 mx-8" style="font-size:17px">COMEDY MOVIES</div>
      </v-row>
  ...
  export default {
    ...
    methods: {
      async fetchComedyMovies() {
        this.isLoading = true;
        try {
          const page1 = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&with_genres=35&language=en-US&sort_by=popularity.desc&page=${Math.floor(Math.random() * 50) + 1}`);
          const page2 = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&with_genres=35&language=en-US&sort_by=popularity.desc&page=${Math.floor(Math.random() * 50) + 1}`);
          
          this.comedyMovies = [...page1.data.results, ...page2.data.results].slice(0, 20).map(movie => ({
            id: movie.id,
            title: movie.title,
